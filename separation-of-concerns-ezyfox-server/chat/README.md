...
1. chat-app-api: contains app's request controllers, app's event handlers and components that are related to
   app
2. chat-app-entry: contains AppEntryLoader class (you should not add any classes in this module)
3. chat-app-entry/config/config.properties: app's configuration file
4. chat-common: contains components that are used by both app and plugin
5. chat-plugin: contains plugin's event handlers, plugin's request controllers and components that are related
   to plugin. You will need handle `USER_LOGIN` event here
6. chat-plugin/config/config.properties: plugin's configuration file
7. chat-startup: contains ApplicationStartup class to run on local (you should not add any classes in this
   module)
8. chat-startup/src/main/resources/logback.xml: logback configuration file
...
You just move to `chat-startup` module and run `ApplicationStartup`
...
<zone>
	<name>chat</name>
	<config-file>chat-zone-settings.xml</config-file>
	<active>true</active>
</zone>
```
...
2. Move to `chat-startup/deploy` folder
...
1. chat-app-api => `ezyfox-server/apps/common/chat-app-api-1.0-SNAPSHOT.jar`
2. chat-app-entry => `ezyfox-server/apps/entries/chat-app`
3. chat-common => `ezyfox-server/common/ chat-common-1.0-SNAPSHOT.jar`
4. chat-plugin => `ezyfox-server/plugins/chat-plugin`
...
You just need use tool or `scp` to copy `chat-startup/deploy` to your server

# How to test?

On your IDE, you need:

1. Move to `chat-startup` module
...
3. Run class `org.youngmonkeys.chat.tools.ExternalLibrariesExporter` in `chat-startup/src/test/java`
